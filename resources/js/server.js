...
function serverHandler(request, response) {
    ...
    try {
        ...
        try {
            ...
            uri = '' //url.parse(request.url).pathname; //url default /demos/
//'resources/js/Dashboard/WelcomeComponent.vue';
        ...
        var matched = false;
        ['/geral/', '/dev/', '/dist/', '/socket.io/', '/node_modules/canvas-designer/', '/admin/'].forEach(function(item) {
        ...
        try {
            ...
            if (filename.search(/geral/g) === -1 && filename.search(/admin/g) === -1 && stats.isDirectory() && config.homePage === '/geral/index.html') {
        ...
        try {
            if (fs.statSync(filename).isDirectory()) {
                ...
                if (filename.indexOf(resolveURL('/geral/MultiRTC/')) !== -1) {
                    filename = filename.replace(resolveURL('/geral/MultiRTC/'), '');
                    filename += resolveURL('/geral/MultiRTC/index.html');
                ...
                } else if (filename.indexOf(resolveURL('/geral/dashboard/')) !== -1) {
                    filename = filename.replace(resolveURL('/geral/dashboard/'), '');
                    filename += resolveURL('/geral/dashboard/index.html');
                } else if (filename.indexOf(resolveURL('/geral/video-conference/')) !== -1) {
                    filename = filename.replace(resolveURL('/geral/video-conference/'), '');
                    filename += resolveURL('/geral/video-conference/index.html');
                } else if (filename.indexOf(resolveURL('/geral')) !== -1) {
                    filename = filename.replace(resolveURL('/geral/'), '');
                    filename = filename.replace(resolveURL('/geral'), '');
                    filename += resolveURL('/geral/index.html');
                } else {
        ...
        if (filename.toLowerCase().indexOf('.php') !== -1) {
